...
const getURL = (language, day, month, all) => {
    ...
    const baseUrl = `https://${language}.wikipedia.org/wiki/${
        all ? '' : locales[language].mainEventPage
    }`;
    return `${baseUrl}${locales[language].template(day, month)}`;
};

const splitDateFromContent = (elem, html, language) => {
    const dateSep = (html
        ? locales[language].dateSeparator
        : htmlDecode(locales[language].dateSeparator)
    );
...
const html2json = (body, language, html) => {
    const $ = cheerio.load(body);
    return $('.mw-parser-output > ul > li').get().map((e) => {
        const elem = $(e);
        const date = parseInt(elem.text().split(locales[language].dateSeparator)[0], 10);
        ...
        return { date, categorie, events: [splitDateFromContent(elem, html, language)] };
    }).reduce((acc, elem) => {
...
module.exports = async ({ lang, day, month, html, all }) => {
    ...
    return (url ? rp({ uri: url, transform: body => html2json(body, lang, html) }) : null);
};
